...
def run(opt_type,learning_rate,train_batch_size,eval_batch_size,train_epochs,momentum,_seed,_run):
    ...
    #TODO decide on what model to use for feature extraction and classfication
    #     the problem now is that there is no readily available easily importable matchboxnet model.
    #     we could use quartz net that is available in torchcv but it has a lot of parameters
    model = M5(n_input=1,n_channel=35)
    ...
    cl_strategy = SupervisedTemplate(
                        model, optimizer,
                        CrossEntropyLoss(), train_mb_size=train_batch_size,
                        device="cuda" if torch.cuda.is_available() else "cpu",
                        train_epochs=train_epochs, eval_mb_size=eval_batch_size,
                        evaluator=eval_plugin,plugins=[]#we can add multiple CL strategies with plugins
                )
    ...
    df_cm = pd.DataFrame(cf_matrix, index = [i for i in DATASET.labels_names],
